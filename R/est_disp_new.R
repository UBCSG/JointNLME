...
est_disp_new <- function(param, other_param,
                          RespLog, 
                          new_dataList,
                          Bi_df, invSIGMA,
                          lower = -Inf, upper = Inf,
                          ...
                          fix_pars, REML, all_glmeObjects,
                          Silent = T, naive, adjust){
  ...
  ff_adj <- function(xx, ...){
    ...
    # exp_dataList[[4]][all_glmeObjects[[4]]$response] <-
    #   exp(eval_fn_row(all_glmeObjects[[4]]$reg_equation, exp_dataList[[4]], par_val, get_gradient = F))
    ...
    #   filter(time_decay <= time + treatment_stop)
    # exp_dataList[[2]] <- exp_dataList[[2]] %>%
    #   filter(time_rebound > time + treatment_stop) %>%
    #   mutate(time_rebound = time_rebound - (time + treatment_stop))
    ...
    if(REML == FALSE){
      log_det <- lapply(1:n, function(i){
        h_i <- -(matrix(as.numeric(hhh[i, -1]), q2, q2) + nD3[[i]])
        log(det(h_i))
      }) %>% unlist() %>% sum()
    } else {
  ...
  if (adjust == TRUE){
    res <- try(optim_iterate_nlminb(str_val, ff_adj, gr = NULL, lower = lower_new, upper = upper_new,
    ...
    res <- optim_iterate_nlminb(str_val, ff, gr = NULL, lower = lower_new, upper = upper_new,
                                check = 1 - as.numeric(Silent), rel_tol = 1e-3, x_tol = 1e-3,
                                # extra info
                                param = param, other_param = other_param,
                                Hmats = Hmats, new_dataList = new_dataList,
                                RespLog = RespLog, Bi_df = Bi_df,
